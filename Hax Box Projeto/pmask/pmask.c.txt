...
#			undef BLAH_WRITE
	...
	void draw_allegro_pmask_inverted(PMASK_CONST PMASK *mask, BITMAP *destination, int dx, int dy, int color) {
		int sx, sy, x, y, w, h;
		if (dx >= destination->w) return;
		if (dy >= destination->h) return;
		w = mask->w;
...
int bsf ( PMASK_WORD_TYPE a ) {
	int r = 0;
	if (!a) return -1;
	if (sizeof(PMASK_WORD_TYPE) > 8) {
		while (!(a & 0xffffFFFF)) {
			r += 32;
			a >>= 32;
		}
	}
	else if (sizeof(PMASK_WORD_TYPE) > 4) {
		if (!(a & 0xffffFFFF)) {
			r += 32;
			a >>= 32;
		}
	}
	if (sizeof(PMASK_WORD_TYPE) > 2 && !(a & 0xFFFF)) {
		r += 16;
		a >>= 16;
	}
	if (sizeof(PMASK_WORD_TYPE) > 1 && !(a & 0xFF)) {
		r += 8;
		a >>= 8;
	}
	if (!(a & 0xF)) {
