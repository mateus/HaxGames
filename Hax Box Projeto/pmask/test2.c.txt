...
int main (int argc, char **argv) {
	...
	for (i = 0; i < 64; i += 1) {
		...
		init_allegro_pmask(&mask[i], sprite[i]);
		if (!tmpbmp) {
	...
	while (!key[KEY_ESC]) {
		...
		if (ctime >= last_graphics + average_graphics_time * 2 + 0.010) {
			...
			textprintf(vscreen, font, 0, ty+=15, palette_color[15], 
				"This is a demonstration program for PMASK (Pixel MASK) collision detection library");
			textprintf(vscreen, font, 0, ty+=15, palette_color[15], 
				"..and the COLlision LIST library included in this version of PMASK");
			textprintf(vscreen, font, 0, ty+=15, palette_color[15], 
