...
int main (int argc, char **argv) {
	...
	install_pmask();
	init_allegro_pmask(&pmask[0], bmp[0]);
	init_allegro_pmask(&pmask[1], bmp[1]);
...
//	readkey();
	...
	while (!key[KEY_ESC]) {
		...
		colliding = check_pmask_collision (&pmask[0], &pmask[1], x[0], y[0], x[1], y[1]);
		...
		for (i = 0; i < 2; i += 1) {
			if (mode[i] == 0) 
				...
				draw_allegro_pmask_stretch(&pmask[i], screen, x[i], y[i], bmp[i]->w, bmp[i]->h, makecol(0,0,144));
			}
		}
	...
	deinit_pmask(&pmask[0]);
	deinit_pmask(&pmask[1]);
